...
\begin{equation}\label{xdot_two}
    \dot{x}_1 = \frac{1}{m_1} \left(p_{x_1} \gsign{+} \alpha_x\right)
\end{equation}

\begin{equation}\label{xdot_two_p2}
    \dot{x}_2 = \frac{1}{m_2} \left(p_{x_2} \gsign{-} \alpha_x\right)
\end{equation}

and

\begin{equation}\label{pdot_two}
    \dot{p}_{x_1} = \frac{q_1 q_2}{r_{12}^2}\left[\frac{(x_1 - x_2)}{r_{12}}\left(1 \gsign{+} \frac{3\dot{r}_{12}^2}{2 c^2}\right) \gsign{-} \frac{\dot{r}_{12}(\dot{x}_1 - \dot{x}_2)}{c^2}\right]
\end{equation}
...
\begin{equation}\label{xdot_expanded}
    \boxed{\dot{x}_i = \frac{1}{m_i} \left(p_{x_i} \gsign{+} \sum_{\substack{j=1 \\ j \neq i}}^{n} \frac{q_i q_j}{c^2} \frac{\dot{r}_{ij} (x_i - x_j)}{r_{ij}^2}\right)}
\end{equation}
...
\begin{equation}\label{pdot_expanded}
    \boxed{\dot{p}_{x_i} = \sum_{\substack{j=1 \\ j \neq i}}^{n} \frac{q_i q_j}{r_{ij}^2}\left[\frac{(x_i - x_j)}{r_{ij}}\left(1 \gsign{+} \frac{3\dot{r}_{ij}^2}{2 c^2}\right) \gsign{-} \frac{\dot{r}_{ij}(\dot{x}_i - \dot{x}_j)}{c^2}\right]}
\end{equation}
...
Perhaps the most promising direction for future research lies in the remarkable fact that Weber's force law admits \textit{attraction between like charges} below a critical separation $\rho = q_1 q_2/(\mu c^2)$, where $\mu = m_1 m_2 / (m_1 + m_2)$ is the reduced mass. \wrongtext{At this critical radius, the sign of the force between \mbox{$q_1$} and \mbox{$q_2$} is reversed.} For $r < \rho$, the particles respond to the still-repulsive Coulomb force as if they had ``negative inertia'', accelerating toward each other rather than away. Weber himself identified this phenomenon in his Sixth Memoir (1871)~\citep{assis-weber-vol4}.
