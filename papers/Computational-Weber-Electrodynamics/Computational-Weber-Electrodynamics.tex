...
We adopt the term \textit{relational magnitudes} from \citep{assis-relational-mechanics} for magnitudes that are intrinsic to the system. Relational magnitudes have the same value in all reference frames and for all observers, even non-inertial ones. The relational magnitudes in Weber's electrodynamics are $\vec{r}$, $r$, $\dot{r}$, $\ddot{r}$ and $\hat{r}$.
