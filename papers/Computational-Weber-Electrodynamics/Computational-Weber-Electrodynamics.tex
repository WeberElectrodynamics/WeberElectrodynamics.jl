...
\begin{abstract}
    We describe the numerical simulation of $n$-body systems in Weber's electrodynamics. We derive the general Hamiltonian and equations of motion for $n$ particles and apply a symplectic integrator. This method enables investigations of the kinematics and dynamics given some initial conditions and the numerical verification of conserved quantities like energy, momentum and angular momentum. Furthermore, it opens the door to exploring alternative models for electromagnetic radiation and bound states of like charges below Weber's critical radius.
\end{abstract}
...
Weber's electrodynamics is an \textit{action at a distance} theory. It satisfies Newton's third law of motion in the \textit{strong form} and thus $\vec{F}_{21} = -\vec{F}_{12}$ is true in general. Therefore, energy, linear momentum and angular momentum are always conserved. Weber's force law~\eqref{force} generalizes Coulomb's law for electrostatics to include \textit{velocity dependent} (magnetic) and \textit{acceleration dependent} (induction) forces. This general setup has a significant \textit{computational advantage} compared to Maxwell-Heaviside's field formalism together with the Lorentz force law. We also have $\vec{F}_{12} = m_1\vec{a}_1$ and $\vec{F}_{21} = m_2\vec{a}_2$, where $m_1$ and $m_2$ are the \textit{inertial masses} of the particles and $\vec{a}_1$ and $\vec{a}_2$ are their accelerations with respect to an \textit{inertial frame of reference}. We adopt the \textit{Universal material frame of reference} described in Relational Mechanics \citep{assis-relational-mechanics} as our inertial reference frame.

Helmholtz argued in the 19th century that Weber's model violates the \textit{principle of conservation of energy}, see \citet{assis-weber-vol4}. Weber replied with great detail to these claims and proved energy conservation extensively in his 6th and 7th memoirs. \citep{assis-weber-vol4} Weber proved that his force~\eqref{force} can be derived from the potential~\eqref{potential}. Therefore, it is a conservative force and the work done is path-independent. The Euler-Lagrange formalism indicates that Weber's electrodynamics is consistent with the principles of mechanics, see equation~\eqref{euler_lagrange}. Nowadays, Weber's electrodynamics is said to be valid only for small velocities (\textit{quasi-static approximation}) compared to $c$ and is not suitable for describing electromagnetic radiation. This might be the biggest point of critique against Weber's electrodynamics. Yet, the third term (acceleration dependent term) in~\eqref{force} naturally falls off as $1/r$, leaving the door open for alternative theories of radiation.

To test the strengths and weaknesses of Weber's electrodynamics, we implement a \textit{symplectic numerical integrator} for investigating $n$-body systems of charged particles. This allows us to explore the dynamics of charged particles interacting via Weber's force law~\eqref{force} in various configurations and scenarios. Important aspects like energy, linear momentum and angular momentum conservation can be verified numerically. Furthermore, it paves the way for studying complex systems with oscillatory behavior and wave phenomena within the context of Weber's electrodynamics. Perhaps most interestingly, Weber's force law admits \textit{attraction between like charges} below a critical distance. We can explore the dynamics below \textit{Weber's critical radius}, which may one day lead to a purely electrodynamic planetary model of the atom. \citep{assis-planetary-model}.
...
Another common system of units is obtained by setting $c = 1$ and (in case of SI units) $1/(4\pi \epsilon_0) = 1$. This system is often used in computational physics for the same reasons mentioned above.
...
is both a function of the positions and momenta after the Legendre transformation. Therefore, standard symplectic methods methods cannot be applied directly. Recent work by \citep{Pihajoki2015} and \citep{Tao2016} introduced explicit symplectic methods using the clever extended phase space method introduced by \citep{Pihajoki2015}. Building on these approaches, \citep{Jayawardana2023} developed a semi-explicit symplectic integrator for non-separable Hamiltonian systems that also preserves quadratic invariants, see \citep{Ohsawa2023}. As of today, this is one of the most suitable methods for our purposes. We describe it briefly here.
...
The computational framework presented in this paper enables the numerical simulation of $n$-body systems governed by Weber's electrodynamics. Beyond validating conservation laws and testing the integrator, this framework opens the door to simulating electronic systems entirely from first principles --- from passive analog components such as coils, capacitors and transformers to more general electronic circuitry --- using a direct particle-interaction model rather than field-based methods. The acceleration-dependent term in Weber's force~\eqref{force} falls off as $1/r$, which leaves room for developing an alternative, action-at-a-distance theory of radiation that can be explored and tested computationally.

Perhaps the most promising direction for future research lies in the remarkable fact that Weber's force law admits \textit{attraction between like charges} below a critical separation $\rho = ee'/(\mu c^2)$, where $\mu$ is the reduced mass. At this critical radius, the effective inertial mass $\mu_{\text{eff}} = \mu(1 - \rho/r)$ changes sign: for $r < \rho$, the particles respond to the still-repulsive Coulomb force as if they had negative inertia, accelerating toward each other rather than away. Weber himself identified this phenomenon in his Sixth Memoir (1871) and called the resulting bound oscillatory state a ``molecular movement''~\citep{assis-weber-vol4}.

\citet{Frauenfelder2024} have provided a complete classification of all classical trajectories in this sub-critical regime and a geometric reinterpretation: the Weber plane carries a metric $g_{rr} = (r - \rho)/r$ that is Riemannian for $r > \rho$ but Lorentzian for $r < \rho$, with the radial direction becoming timelike below the critical radius. This metric signature change is the geometric manifestation of the negative effective mass. Crucially, their analysis shows that classical collisions at the origin are regularizable only when the angular momentum $\ell = 0$, corresponding to purely radial (head-on) motion. For $\ell \neq 0$, particles spiral into the origin at infinite speed in finite time, and no smooth coordinate transformation can remove this singularity --- the infinite winding number is a topological obstruction preserved by all regularization techniques.

This constraint poses a fundamental challenge: finding stable, bound configurations of like charges below the critical radius that go beyond purely radial oscillation. One avenue is to develop advanced regularization or stabilization techniques that could handle the $\ell \neq 0$ singularity. Another, perhaps more promising, approach is to expand our view beyond the isolated two-body problem and consider multi-particle systems --- for instance, balancing a pair of like charges in the sub-critical regime with oppositely charged particles above $\rho$, or exploring other conceivable configurations whose mutual interactions provide the stabilizing forces that a single pair lacks. The goal, in all cases, is to find dynamically stable bound states of charged particles.

This line of investigation is part of a long-standing program that originated with Weber himself and his planetary model of the atom~\citep{assis-planetary-model}, in which bound pairs of like charges form atomic nuclei while unlike charges orbit under the combined electrodynamic forces. With the computational tools presented here, and with modern AI-assisted methods that can efficiently search the space of possible configurations --- automating algebraic manipulations, symmetry analysis and systematic testing by numerical integration --- one can explore this vast landscape of possibilities more thoroughly than was previously feasible. The ultimate goal of this program would be to uncover the electrodynamic origins of the periodic table of elements.
...
We adopt the term \textit{relational magnitudes} from \citep{assis-relational-mechanics} for magnitudes that are intrinsic to the system. Relational magnitudes have the same value in all reference frames and for all observers, even non-inertial ones.
...
I would like to thank Norman Wildberger, Andre Assis, Koen van Vlaenderen and Ton Kuiper for educating me.
