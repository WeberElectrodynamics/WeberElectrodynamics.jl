...
% ---- REVIEW MARKUP -- remove once the corrections are accepted ----
% wrong env / \wrongtext{}  red : wrong as written, left uncorrected
% \gsign{}  green : a sign corrected in this pass
\usepackage[most]{tcolorbox}
\definecolor{revred}{RGB}{255,196,203}
\definecolor{revgreen}{RGB}{183,232,193}
\newcommand{\gsign}[1]{\mathbin{{\setlength{\fboxsep}{1.5pt}\colorbox{revgreen}{\ensuremath{#1}}}}}
\usepackage{soul}
\newcommand{\wrongtext}[1]{{\sethlcolor{revred}\hl{#1}}}
\newtcolorbox{wrong}{breakable,colback=revred,colframe=revred!65!black,
...
Before we can apply Hamilton's canonical equations, we express the Hamiltonian in terms of \textit{positions} and \textit{momenta}. The canonical momentum $p_{x_1}$ is defined as

\begin{equation}\label{px1}
    p_{x_1} = \frac{\partial L}{\partial \dot{x}_1} = m_1 \dot{x}_1 - \frac{q_1 q_2}{c^2} \frac{\rdot (x_1 - x_2)}{r^2}
\end{equation}

The \textit{Legendre transform} then yields

\begin{equation}\label{H_p}
    H = H(x_1, y_1, z_1, x_2, y_2, z_2, p_{x_1}, p_{y_1}, p_{z_1}, p_{x_2}, p_{y_2}, p_{z_2})
\end{equation}

where each velocity component in~\eqref{H} is replaced by expressions like \eqref{xdot_two} and \eqref{xdot_two_p2}.
...
with analogous definitions for $\alpha_y$ and $\alpha_z$, we obtain
...
(and analogous equations for the $\dot{y}_1$, $\dot{y}_2$, $\dot{z}_1$, and $\dot{z}_2$ components) by either rewriting the canonical momenta \eqref{px1} or by applying \eqref{xdot} to \eqref{H_p}.

We also apply~\eqref{pdot} to obtain
...
Perhaps the most promising direction for future research lies in the remarkable fact that Weber's force law admits \textit{attraction between like charges} below a critical separation $\rho = q_1 q_2/(\mu c^2)$, where $\mu = m_1 m_2 / (m_1 + m_2)$ is the reduced mass. Below this critical radius, the sign of the force between $q_1$ and $q_2$ is reversed. For $r < \rho$, the particles respond to the still-repulsive Coulomb force as if they had ``negative inertia'', accelerating toward each other rather than away. Weber himself identified this mathematical phenomenon in his Sixth Memoir (1871)~\citep{assis-weber-vol4}.
