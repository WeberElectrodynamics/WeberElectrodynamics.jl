...
where each velocity component $\dot{x}_i$ in~\eqref{H} is replaced by $p_{x_i}/m_i$ and similarly for all other velocity components. Because the Weber potential depends on $\dot{r}_{12}$, this substitution is an approximation: it produces a Hamiltonian $H \approx H_{\mathrm{exact}}$ that agrees with the exact Legendre transform to $\mathcal{O}(v^2/c^2)$, which is sufficient for the quasi-static regime $v \ll c$.
