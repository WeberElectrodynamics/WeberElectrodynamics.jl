...
\citet{Frauenfelder2024} have provided a complete classification of all classical trajectories in this sub-critical regime and a striking geometric reinterpretation: the Weber plane carries a metric $g_{rr} = (r - \rho)/r$ that is Riemannian for $r > \rho$ but Lorentzian for $r < \rho$, with the radial direction becoming timelike below the critical radius. This metric signature change is the geometric manifestation of the negative effective mass. Crucially, their analysis shows that classical collisions at the origin are regularizable only when the angular momentum $\ell = 0$, corresponding to purely radial (head-on) motion. For $\ell \neq 0$, particles spiral into the origin at infinite speed in finite time, and no smooth coordinate transformation can remove this singularity --- the infinite winding number is a topological obstruction preserved by all regularization techniques.
