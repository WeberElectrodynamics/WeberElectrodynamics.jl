...
\subsection{Comparison with Maxwell's electrodynamics}
In the Maxwell-Heaviside framework, the electromagnetic state is described by two source densities (charge density $\rho$ and current density $\vec{J}$), two potentials (scalar $\phi$ and vector $\vec{A}$) and two vector fields ($\vec{E}$ and $\vec{B}$) derived from them via $\vec{E} = -\nabla\phi - \partial\vec{A}/\partial t$ and $\vec{B} = \nabla \times \vec{A}$. Computing the Lorentz force on a charged particle requires the fields, which in turn require the potentials, giving a total of ten degrees of freedom per point in space. These fields are defined over all of three-dimensional space and must be discretized on a computational grid. Standard methods such as FDTD (finite-difference time-domain) require $\mathcal{O}(N_\text{grid})$ operations per timestep, where $N_\text{grid} = (L/\Delta x)^3$ is the number of grid cells needed to resolve the simulation domain.

By contrast, Weber's action-at-a-distance formulation requires no fields, no spatial grid and no retardation. The entire computational cost per timestep is $\mathcal{O}(n^2)$ pair evaluations, each involving only elementary arithmetic, see Section~\ref{n-body-hamiltonian}. Moreover, Weber's Hamiltonian \eqref{hamiltonian} is a finite-dimensional function of particle positions and momenta, which makes it directly amenable to symplectic numerical integration as described in Section~\ref{symplectic}. The Maxwell field Lagrangian, by contrast, is an infinite-dimensional functional. Standard Maxwell solvers such as FDTD and PIC methods are not derived from a Hamiltonian and do not in general preserve a symplectic structure.
