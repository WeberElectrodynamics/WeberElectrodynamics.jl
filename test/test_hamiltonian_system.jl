@testset "Weber System" begin
    ...
    @testset "Two-particle force with nonzero momentum" begin
        ...
        sys2.dp_dt_compiled(out_p, q, p, 0.0, params2)
    ...
    @testset "Energy function correctness" begin
        ...
        E_compiled = system.hamiltonian_compiled(q, p, 0.0, [m1, m2, q1, q2, c])
    ...
    @testset "compiled EOMs are autonomous (t-arg invariance)" begin
        ...
        out_q_ref = zeros(4);
        out_p_ref = zeros(4)
        sys.dq_dt_compiled(out_q_ref, q, p, 0.0, params)
        sys.dp_dt_compiled(out_p_ref, q, p, 0.0, params)
        H_ref = sys.hamiltonian_compiled(q, p, 0.0, params)

        for t_val in (1.7, -0.5, 1e6, π)
            out_q = zeros(4);
            out_p = zeros(4)
            sys.dq_dt_compiled(out_q, q, p, t_val, params)
            sys.dp_dt_compiled(out_p, q, p, t_val, params)
            H_val = sys.hamiltonian_compiled(q, p, t_val, params)
